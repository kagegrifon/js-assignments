...
function getRectangleString(width, height) {
    if (width < 2 || height < 2) throw new Error('Minimum size of string rectangle is 2 x 2');
...
function encodeToRot13(str) {
    const SOURCE_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    const OUTPUT_ALPHABET = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    const outputStr = str.replace(/\w/g, (match) => {
        return OUTPUT_ALPHABET[SOURCE_ALPHABET.indexOf(match)];
    });
    return outputStr;
}
...
function getCardId(value) {
    ...
    return SUITS.indexOf(value.slice(-1)) * 13 + CARD_VALUES.indexOf(value.slice(0, -1));
}
